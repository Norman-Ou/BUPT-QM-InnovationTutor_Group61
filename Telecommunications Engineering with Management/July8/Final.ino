...
int G_Brightness = 0;
int B_Brightness = 0;
int lightType = 3; //0 -> R; 1 -> G; 2 -> B; 3 ->RGB
int HOUR=23;
int MINUTE=59;
int SECOND=50;//设置初始时间


void lightOn()
{
    lightType = lightType % 3; //0 -> R; 1 -> G; 2 -> B; 3 ->RGB
...
void loop()
{
  ...
  if (AutoLight)
  {
    ...
    //analogWrite(10, EPBrightness);
    //analogWrite(11, EPBrightness);
  ...
  //报警系统
  if (digitalRead(3) == LOW || distance <= 10 || digitalRead(4) == HIGH || analogRead(A0) > 65)
  {
    Alarm();
    webWarning = true;
  }
